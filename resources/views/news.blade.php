            ...
            <div class="row justify-content-center">
                        ...
                        <form action="{{ route('news') }}" method="GET">
                            <div class="form-group">
                                <input type="text" name="search" class="form-control"
                                    placeholder="Search by title, Topics, or tag"
                                    value="{{ request('search') }}">
                            </div>
                            <button type="submit" class="subscribe-btn btn-style-one">Search</button>
                        </form>
                    </div>

                    <div class="mobile-topics d-block d-md-none mt-3 mb-4">
                        <!-- Dropdown for Mobile -->
                        <h3>News Topics</h3>
                        <select id="newsCategoryDropdown" class="form-control d-block d-md-none">
                            @if ($newscategories->isNotEmpty())
                                ...
                                @foreach ($newscategories as $category)
                                    <option value="{{ route('particularcat', $category->slug) }}"
                                        data-id="{{ $category->id }}">
                ...
                <div class="col-lg-8">
                        @php
                            ...
                            $categories = \App\Models\NewsCategory::with(['posts' => function ($query) use ($search, $tag) {
                                $query->when(!empty($search), function ($query) use ($search) {
                                        $query->where(function ($q) use ($search) {
                                            $q->where('title', 'LIKE', "%{$search}%")
                                              ...
                                              ->orWhereHas('newscategory', function ($categoryQuery) use ($search) {
                        ...
                        @foreach ($categories as $category)
                            @php 
                                $posts = $category->posts->sortByDesc('posted_date')->take(3)->values(); 
                            @endphp
                                @if($posts->isNotEmpty()) 
                                    <!-- Section Title -->
                                    <div class="d-flex align-items-center mb-2" style="margin-left: 2%;margin-right: 2%;">
                                        <a class="category-click"  data-id="{{ $category->id }}" data-url="{{ route('particularcat', $category->slug) }}" href="javascript:void(0);">{{ $category->title }}</a>
                                    ...
                                    @foreach ($posts as $key => $post)
                                        <!-- Static News Block -->
                                        <div class="news-block col-lg-12 col-sm-12">
                                                ...
                                                <div class="lower-content ml-3">
                                                    ...
                                                    <p>
                                                        ...
                                                        @if(strlen($content) > 10)
                                                            @if(str_starts_with($post->slug, 'http'))
                                                    ...
                                                    <div class="social-share-buttons">
                                                        <a href="https://www.facebook.com/sharer/sharer.php?u={{ urlencode(route('news-detail', $post->slug)) }}" target="_blank" class="btn btn-primary btn-sm">
                                                            <i class="fab fa-facebook-f"></i> Share
                                                        </a>
                                                        <a href="https://twitter.com/intent/tweet?url={{ urlencode(route('news-detail', $post->slug)) }}&text={{ urlencode($post->title) }}" target="_blank" class="btn btn-info btn-sm ml-2">
                                                            <i class="fab fa-twitter"></i> Share
                                                        </a>
                                                        <a href="https://www.linkedin.com/sharing/share-offsite/?url={{ urlencode(route('news-detail', $post->slug)) }}" target="_blank" class="btn btn-primary btn-sm ml-2">
                                        ...
                                        @if ($key == 2) 
                                            <div class="news-block col-lg-12 col-sm-12 text-center" style="margin-bottom:4% !important;">
                                                <a href="{{ route('particularcat', $category->slug) }}" class="read-more-post">Read more {{ $category->title }} articles</a>
                                            </div>
                                        @endif                                        
                                    @endforeach
                                @endif
                ...
                <div class="col-lg-4 search-category-section">
                    <div class="search d-none d-md-block">
                        <form action="{{ route('news') }}" method="GET">
                            <div class="form-group">
                                <input type="text" name="search" class="form-control" placeholder="Search by title, Topics, or tag" value="{{ request('search') }}">
                            </div>
                            <button type="submit" class="subscribe-btn btn-style-one">Search</button>
                        </form>
                    </div>
                    <!-- Static News Categories Section -->
                    <div class="news-categories mb-5 mt-2 d-none d-md-block">
                        <h3>News Topics</h3>
