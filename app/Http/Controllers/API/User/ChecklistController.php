...
class ChecklistController extends Controller
{
        ...
        try {
            ...
            else
            {
                $param = array(
                    ...
                    'status' => $request->submissionType,
                    'checklist_meta' => !empty($request->checklist_meta) ? json_encode($request->checklist_meta) : ''
                );
    ...
    public function getUserChecklistDetail(Request $request)
    {
     ...
     ## Function to get facility compliances
    public function getCompliances(Request $request)
    {
        ...
        if ($validator->fails()) {
            return response()->json(['status' => false, 'message' => "Validation error occurred"]);
        }

        try {
            // Start the query to fetch compliance records
            ...
            $uChecklist = $uChecklist->get()->toArray(); // No need to format the created_at date

            // Prepare the response
            $result = [
                'status' => true,
                'message' => count($uChecklist) . " Record(s) found",
                'data' => $uChecklist,
            ];
        } catch (Exception $e) {
            $result = [
    ...
    ## Function to get user completed checklist
    public function getCompletedChecklist(Request $request) 
    {
        // Validate the incoming request
        $validator = Validator::make($request->all(), [
            'user_id' => 'required',
        ...
        try {
            $completed_checklists = DB::table('compliance_checklists as cc')
                    ->join('user_compliance_checklists as ucc', 'ucc.checklist_id', '=', 'cc.id')
                    ->leftJoin('users as u', 'u.id', '=', 'cc.created_by')
                    ->select('cc.id', 'cc.slug', 'cc.title', 'ucc.created_at as submitted_on', 'ucc.updated_at as updated_on')
                    ->where('cc.status', 1)
                    ->where('cc.deleted_at', NULL)
                    ->where('ucc.user_id', $request->user_id)
                    ->where('ucc.status', 1)->get()->toArray();

            $result = [
                'status' => true,
                'message' => count($completed_checklists) . " Record(s) found",
                'data' => $completed_checklists,
            ];
