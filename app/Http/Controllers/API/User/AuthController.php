...
class AuthController extends Controller
{
    ...
    public function register(Request $request)
    {
        ...
        try {
            ...
            if ($user->id) {
                ...
                # Send notification email to master admin
                ...
                if(isset($admin_email->field_value) && !empty($admin_email->field_value))
                {
                    $param = [
                        'admin_name' => 'Admin',
                        'full_name' => strip_tags($request->name),
                        'email' => strip_tags($request->email),
                        'admin_login_url' => config('custom.admin_login_url'),
                        'admin_email' => $admin_email->field_value,
                    ];
                    Mail::send('emails.admin.candidate-register', $param, function ($message) use ($param) {
                        $message->subject(config('custom.admin_candidate_register'));
                        $message->to($param['admin_email']);
                    });
                }
    ...
    public function login(Request $request)
    {
        ...
        try {
            ...
            $userRecord = User::where('email', request('email'))->where('is_login_allowed',1)->where('deleted_at', NULL)->first();
