    ...
    public function getDashboardData(Request $request)
    {
        ...
        try {
            ...
            $data['totalJobApplications'] = DB::table('job_applications as ja')
                ...
                ->leftJoin('users as u', 'u.id', '=', 'ja.user_id')
                ->where('j.status', 1)
                ->where('j.deleted_at', NULL)
            ...
            $data['recentJobs'] = DB::table('jobs as j')
                ...
                ->select('j.id', 'j.title', 'j.unique_id', 'j.created_at', 's.name as state_name', 's.code as state_code',  'city.city_name', 'j.status', 'u.name as creator_name', 'u.unique_id as creator_unique_id', 'u.role_id as creator_role_id')
                ->where('j.deleted_at', NULL)
                ->where('j.status', 1)
                ->limit(5)
                ->orderBy('j.id', 'desc')
