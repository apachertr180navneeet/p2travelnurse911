    ...
    public function getEvents(Request $request)
    {
        ...
        try {
            ...
            $tasks = DB::table('events as s')
                ...
                ->select('s.id', 's.title', 's.type','s.type_id', 's.start_date','s.end_date','s.start_time','s.end_time', 's.status','s.created_at',
                    'u.name as creator_name',
                    'u.role_id as creator_role_id',
                    'u.unique_id as creator_unique_id',
                    'u2.name as assignee_name',
                    'u2.role_id as assignee_role_id',
                    'u2.unique_id as assignee_unique_id'
                )
    ...
    public function deleteEvent(Request $request)
    {
        ...
        try {
            ...
            if($request->type == 'task')
            {
                ...
                $args['updated_by'] = $request->user_id;
                $args['updated_at'] = $this->entryDate;
                $args['deleted_at'] = $this->entryDate;
                DB::table('tasks')->where('id', $request->type_id)->update($args);
            }
            else if($request->type == 'interview')
            {
                $args = array();
                ...
                $args['updated_at'] = $this->entryDate;
                $args['deleted_at'] = $this->entryDate;
                DB::table('job_interviews')->where('id', $request->type_id)->update($args);
            }
