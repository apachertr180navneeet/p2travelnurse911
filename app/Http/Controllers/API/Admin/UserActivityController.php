...
class UserActivityController extends Controller
{
    ...
    ## Function to get pilot partner enquiries
    public function getPilotPartnerEnquiries(Request $request)
    {
        $validator = Validator::make($request->all(), [
            'user_id' => 'required',
        ]);
        ...
        try {
            ...
            if (isset($request->keyword) && !empty($request->keyword)) {
                 $activities->where(function ($query) use ($request) {
                    $query->where('ce.name', 'LIKE', "%{$request->keyword}%")
                          ->orWhere('ce.company_name', 'LIKE', "%{$request->keyword}%")
                          ->orWhere('ce.type_of_company', 'LIKE', "%{$request->keyword}%")
                          ;
                });
            } 
            if ((isset($request->start_date) && !empty($request->start_date)) || isset($request->end_date) && !empty($request->end_date)) {
