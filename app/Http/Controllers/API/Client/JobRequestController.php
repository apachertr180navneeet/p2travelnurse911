...
class JobRequestController extends Controller
{
    ...
    public function getMatchedJobRequestList(Request $request)
    {
        ...
        try {
            ...
            $userData = User::getUserDetailByID($request->client_id);
            if ($userData->role_id == User::ROLE_OFFICE_ADMIN) {
            ...
            if ($request->filled('state_id') && !empty($request->state_id)) {
                $matchedRequests->where('job_request_state_cities.state_id', $request->state_id);
            }
            ...
            if ($request->filled('is_hide') && $request->is_hide != 0) {
    ...
    public function getAllJobRequest(Request $request)
    {
        ...
        try {
            ...
            if ($request->filled('state_id') && !empty($request->state_id)) {
                $unmatchedRequests->where('job_request_state_cities.state_id', $request->state_id);
            }
