...
class DocumentController extends Controller
{
        ...
        try {
            ...
            if (isset($request->keyword) && !empty($request->keyword)) {
                 $documents->where(function ($query) use ($request) {
                    $query->where('d.title', 'LIKE', "%{$request->keyword}%")
                          ->orWhere('dt.doc_name', 'LIKE', "%{$request->keyword}%")
    ...
    ## Function to get user's document
    ...
    public function getDocumentShareHistory(Request $request)
    {
        ...
        try {
            $documents = DB::table('user_documents_share_history as dsh')
                ...
                ->select('dsh.id', 'ud.title','dsh.share_to', 'ud.file_name', 'ud.file_type', 'dsh.created_at','dt.doc_name as doc_type_name','u.name as uploaded_by','u.unique_id as uploaded_by_unique_id','u.profile_pic as uploaded_by_profile_pic');
                
            if (isset($request->keyword) && !empty($request->keyword)) {
                 $documents->where(function ($query) use ($request) {
                    $query->where('ud.title', 'LIKE', "%{$request->keyword}%")
                          ->orWhere('dt.doc_name', 'LIKE', "%{$request->keyword}%")
                          ->orWhere('u.name', 'LIKE', "%{$request->keyword}%")
                          ->orWhere('dsh.share_to', 'LIKE', "%{$request->keyword}%")
                          ;
                });
            } 
    ...
    ## Function to get user document share history
    public function getUserSharedDocuments(Request $request)
    {
        $validator = Validator::make($request->all(), [
            'user_id' => 'required',
            'userID' => 'required',
        ...
        try {
            $documents = DB::table('user_documents_share_history as dsh')
                ...
                ->leftJoin('doc_types as dt', 'ud.doc_type_id', '=', 'dt.id')
                ->leftJoin('users as u', 'dsh.user_id', '=', 'u.id')
                ->select('dsh.id', 'ud.title','dsh.share_to', 'ud.file_name', 'ud.file_type', 'dsh.created_at','dt.doc_name as doc_type_name','u.name as uploaded_by',
                'u.unique_id as uploaded_by_unique_id','u.profile_pic as uploaded_by_profile_pic')
                ->where('dsh.user_id', $request->userID);
