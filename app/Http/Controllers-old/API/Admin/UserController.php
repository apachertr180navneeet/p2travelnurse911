...
class UserController extends Controller
{
    ...
    public function getUsers(Request $request)
    {
        ...
        try {
            ...
            ->select('u.id', 'u.name', 'u.unique_id', 'u.email', 'u.phone', 'u.created_at', 'u.profile_pic', 'u.status', 's.name as state_name', 'city.city_name','u.status as user_status',
            'u2.name as creator_name','u2.role_id as creator_role_id','u2.unique_id as creator_unique_id','j.title as job_title','ud.state_id','ud.city_id','ud.bio',
                    'j.unique_id as job_unique_id','ja.status as job_status','sp.specialty', DB:: raw('GROUP_CONCAT(DISTINCT s2.name ORDER BY s2.name SEPARATOR ", ") as desired_states'),'p.profession')
            ->when($request->role_id == 3 || $request->role_id == 2, function ($query) {
                return $query->leftJoin('clients as c', 'c.user_id', '=', 'u.id')
                             ->addSelect('c.company_name','c.website','c.primary_industry','c.company_size','c.bio as company_bio','c.address_line1','c.address_line2','c.founded_in'); // Add specific column from clients table
            })
    ...
    public function getAllUsers(Request $request)
    {
        ...
        try {
            ...
            $users = DB::table('users as u')
                ...
                'ud.state_id','ud.city_id','ud.bio','u2.name as creator_name','u2.unique_id as creator_unique_id','u2.role_id as creator_role_id',
    ...
    public function getFacilityDetails(Request $request)
    {
        ...
        try {
            ...
            $raw = DB::table('users as u')
                ...
                ->leftJoin('states as s', 'ud.state_id', '=', 's.id')
                ->leftJoin('cities as city', 'ud.city_id', '=', 'city.id')
                ->select('u.*','ud.bio','ud.country_id','ud.state_id','ud.city_id','ud.address_line1','ud.address_line2','s.name as state_name', 'city.city_name','ud.dob','ud.total_experience')
                ->where(['u.unique_id' => $request->userID])
                ->first();
