    ...
    public function getClients(Request $request)
    {
        ...
        try {
            $complianceFiles = DB::table('facilities as f')
                ...
                ->select('f.id', 'f.title', 'f.status', 'f.type','f.state_id','f.city_id','f.created_at','s.name as state_name', 'city.city_name','u.name',
                    'u.unique_id as user_unique_id','u.role_id')
    ...
    public function getAssignments(Request $request)
    {
        ...
        try {
            $complianceFiles = DB::table('assignments as a')
                ...
                ->leftJoin('users as u2', 'a.created_by', '=', 'u2.id')
                ->select('a.confirmed_with_facility','a.confirmed_with_traveler','a.assigned_unit','a.assignment_length','a.shift','a.approved_time_off','a.facility_id','a.start_date','a.end_date','f.title as facility_title','a.id','a.user_id', 'a.status', 'a.state_id','a.city_id','a.created_at','s.name as state_name', 'city.city_name','u.name',
                    'u.unique_id as user_unique_id','u2.name as submitted_name',
                    'u2.unique_id as submitted_unique_id','u2.role_id as submitted_role_id')
